...
class Case:
    ...
    def __init__(
        self, coordonates: Position = Position(), type: str = "Netherrack"
    ):
        '''Creates a Case from a Position and a type, or from a default Position and type if empty.'''
        if type not in Case.types_of_cases:
            raise ValueError(f"type must be one of {Case.types_of_cases}") # returns an error if the type isn't part of the possible types for a case
    ...
    def initFromModel(cls, model):
        ...
        if not isinstance(model, cls):
            raise TypeError("model must be a Case instance") # returns an error if the provided case isn't a case
        return cls(Position(model.getPosition()), model.getType()) # creates a case from the position and type of the model
